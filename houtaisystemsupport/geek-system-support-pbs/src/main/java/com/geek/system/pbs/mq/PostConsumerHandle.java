...
import com.geek.system.pbs.mq.facade.PostMessage;
...
@ConditionalOnProperty(prefix = "app.mq.consumer", value = "enablePost", havingValue = "true", matchIfMissing = true)
@Service("PostConsumerHandle")
public class PostConsumerHandle extends StringAbstractMessageConsumerHandler {

	@Autowired
	private DingDingMsg dingDingMsg;

	@Autowired
	private PostMessageHandleService postMessageHandleService;

	@Autowired
	private RepairOrderConsumerRecordService repairOrderConsumerRecordService;

	@Value("${spring.application.name:PostConsumer}")
	...
	public boolean handler(String topic, String tag, long bornTimestamp, String message) {
		...
		try {
			...
			PostMessage msg = JSON.toJavaObject(json.getJSONObject("message"), PostMessage.class);
			...
			msg.setUniqueId(json.getLong("uniqueId"));
			msg.setBornTimestamp(bornTimestamp);
			postMessageHandleService.handleOrgInfo(msg);
		} catch (Exception e) {
			// 消费失败
			status = MQStatus.CONSUMER_FAIL;
			log.error("消费消息处理职务业务异常: {},消息内容：{}", e.getMessage(), message);
			errMsg = e.getMessage();
			dingDingMsg.sendMessage("消费消息处理职务业务异常", e.getMessage(), message, null);
			...
			workInfo.setHandler("PostConsumerHandle");
			// 保存处理最终结果
