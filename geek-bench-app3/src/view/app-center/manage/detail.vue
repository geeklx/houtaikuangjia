<template>
  <fs-dialog
    ...
    :title="operateType === 'new' ? '创建应用' : '编辑应用'"
    @closed="close"
    width="980px"
  >
    <fs-form
    ...
    >
      ...
      <fs-form-item label="应用来源：" prop="inOrOut">
        <fs-radio-group v-model="formData.inOrOut">
          <fs-radio :label="true">内部原生</fs-radio>
          <fs-radio :label="false">第三方原生</fs-radio>
        </fs-radio-group>
      </fs-form-item>
  ...
  methods: {
    ...
    submitHandler_ () {
      this.$refs[this.validRef].validate(async valid => {
        if (valid) {
          let { maintainMessage, maintainUrl, maintain } = this.formData
          if (maintain) {
            maintainMessage = this.editor.getHTML(this.formData.maintainMessage)
            maintainMessage = `
            <!DOCTYPE html>
            <html lang="">
              <head>
          ...
          `
            const html = new Blob([maintainMessage], { type: 'text/html;charset=utf-8' })
